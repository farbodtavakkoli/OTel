...
**NVIDIA H100 (Hopper, sm_90) — verified 2026-08-22, single-GPU smoke.** Same story, mirror
image: the accelerated backend that resolves this GPU is **`llamacpp:cuda`** (the ROCm
backend reports `Unsupported GPU`, exactly as vLLM did on gfx950). All three workloads run
on **one Lemonade Server on port 8350**, GPU-resident, via the CUDA llama.cpp backend. Small
GGUFs used for a fast smoke (none were cached on this box):

| Leaf | Model (H100 smoke) | Verdict on H100 (sm_90, CUDA 13 host / bundled CUDA 12.9) |
|---|---|---|
...
> The H100 smoke deliberately used **small** GGUFs (0.6B llm, 300M embed, 0.6B rerank), not
> the 27 GB Qwen3.8-27B, because no GGUFs were cached on this box and the point was to prove
> the backend/GPU path, not raw throughput. The install and serving path is identical to the
> 27B; only the checkpoint size differs. See each leaf's **H100** section.

## Install (AMD / ROCm — the route that worked, and the trap)
...
## Install (NVIDIA / CUDA — H100, verified 2026-08-22)
...
backend availability. On this box:
...
only**: exactly symmetric to MI355X, where it was a ROCm llama.cpp front-end only and the
FP8/vLLM path was refused. GGUF completion/embedding/reranking all work and hit the GPU;
...
- Pre-seeding the HF cache (`refs/main` + `snapshots/<sha>/` + registry json) makes
  `lemonade pull` skip re-downloads — saved a 28 GB pull in the campaign.
