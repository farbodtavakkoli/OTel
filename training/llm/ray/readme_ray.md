...
> **Tested topology:**
> - **NVIDIA H100 80GB (CUDA 13.0, driver 580.173.02), August 2026 — WORKS, single GPU, no
...
>   manager handles the rest. Verified at **1 GPU** ([§8.2](#82-h100-cuda-130--tested-august-2026))
...
>   2/8-GPU pass needs.
> - **AMD Instinct MI355X (ROCm 7.2.4), August 2026 — WORKS WITH ONE REQUIRED ENV VAR**
...
>   ([§8.1](#81-mi355x-rocm-724--tested-august-2026)) and then at the full **8 GPUs**
>   ([8-GPU run](#8-gpu-run-8x-mi355x-rocm-724--tested-august-2026)) — `--num_workers 8`
...
> **Tested on H100 / CUDA 13.0 (Aug 2026):** the `torch==2.11.0` pin has **no cu130 wheel**.
...
> and evidence in [§8.2](#82-h100-cuda-130--tested-august-2026).
...
# requirements_ray.txt (evidence blocks below may still reference the old venv name).
...
Ray-on-ROCm bug, explained in [§8.1](#81-mi355x-rocm-724--tested-august-2026).
...
> [§8.1](#81-mi355x-rocm-724--tested-august-2026).
...
- Source: [Ray Accelerator Support documentation](https://docs.ray.io/en/latest/ray-core/scheduling/accelerators.html)
  ...
  checked August 2026). Ray's docs also list TPU (fully tested), and Intel GPU/Gaudi,
...
**Verdict: WORKS WITH CHANGES.** No edits to `train_llm_ray.py` were needed; the change is
...
**Exact run command:**
```bash
source .env_train_llm_ray/bin/activate
...
python3 train_llm_ray.py \
  ...
  --storage_path /mnt/data_1.5t/outputs/train_llm_ray --run_name mi355x_smoke
```
...
```
2026-08-19 10:52:16 - INFO - root - Ray cluster resources: {'GPU': 2.0, 'CPU': 256.0,
  'accelerator_type:AMD-Instinct-MI355X-OAM': 1.0, ...}
(RayTrainWorker pid=4169843) Setting up process group for: env:// [rank=0, world_size=2]
(RayTrainWorker pid=4107769) {'loss': '2.431', 'grad_norm': '18.88', 'learning_rate': '0', 'entropy': '1.717', 'num_tokens': '887', 'mean_token_accuracy': '0.5209', 'epoch': '0.2'}
(RayTrainWorker pid=4107769) {'train_runtime': '9.85', 'train_samples_per_second': '1.015', 'train_steps_per_second': '0.508', 'train_loss': '2.843', 'epoch': '1'}
(RayTrainWorker pid=4107769) Checkpoint successfully created at: Checkpoint(filesystem=local, path=/mnt/data_1.5t/outputs/train_llm_ray/mi355x_smoke2/checkpoint_...)
```
...
```
10:52:39 GPU[5]: GPU use (%): 98 | GPU[6]: GPU use (%): 97
         GPU[5]: VRAM Total Used (B): 22076882944 | GPU[6]: VRAM Total Used (B): 18299920384
10:52:43 GPU[5]: GPU use (%): 96 | GPU[6]: GPU use (%): 96
```
...
**Verdict: WORKS — scales cleanly from 2 to 8 GPUs with no code change and no new package.**
The single required env var from §8.1 is **still required at 8 workers**; everything else is
just `--num_workers 8`. Ray placed 8 workers on 8 **distinct** GPUs, `world_size=8`, finite
decreasing loss, exit code 0, clean teardown.

**Exact launch command** (the runner sources the venv, then overrides the device pins —
see the warning below — and asserts both device counts before training):

```bash
cd training/llm/ray
source .env_train_llm_ray/bin/activate

# The venv's activate script pins HIP/CUDA_VISIBLE_DEVICES=5,6 — override AFTER sourcing.
...
python3 train_llm_ray.py \
  ...
  --max_failures 0 \
  --storage_path /mnt/data_1.5t/outputs/train_llm_ray/gpu8 --run_name mi355x_gpu8
```
...
2026-08-19 18:46:30 - INFO - root - Launching run 'mi355x_gpu8' | workers=8 | max_failures=0
...
2026-08-19 18:47:19 - INFO - root - Training complete. Metrics: {... 'epoch': 15.0, 'step': 30}
=== TRAIN EXIT CODE: 0 ===
```
...
```
18:47:12  GPU use (%):  [0]=53 [1]=51 [2]=52 [3]=49 [4]=47 [5]=47 [6]=45 [7]=45
...
The same PIDs appear as `(RayTrainWorker pid=3076xx)` in the driver log, and `pid=307688`
...
failure) one GPU would show ~29 GB and another ~0. None does. Raw samples:
`/mnt/data_1.5t/outputs/train_llm_ray/gpu8/rocm_smi_inband.txt`.
...
- **`RAY_EXPERIMENTAL_NOSET_HIP_VISIBLE_DEVICES=1` is still required.** The §8.1 root cause
  ...
  `set_device(cuda:7)` in a process narrowed to one device). It was exported for this run.
...
### 8.2 H100 (CUDA 13.0) — tested August 2026
...
export HF_HOME=/mnt/gsma/gsma/gsma/models          # 350M is fully cached here
...
Cluster saw exactly one GPU, and one worker at `world_size=1`:
...
```
(RayTrainWorker pid=1518126) Checkpoint successfully created at:
  Checkpoint(filesystem=local, path=.../h100_smoke/checkpoint_2026-08-23_03-32-47.828948)
...
worker PID `1518126`, GPU 6 UUID `GPU-e4fe48bc-0c29-f21e-6523-759f964bf823`):

```
==== sample 20 ====                       # per-GPU memory.used / util
0, 67124 MiB, 100 %    <- co-tenant prod job (GPUs 0-3), untouched
...
6, 1393 MiB, 0 %       <- OUR worker
7, 0 MiB, 0 %          <- idle, untouched
-- compute-apps --  (pid, used_memory, gpu_uuid)
1273508..1273511  ...  <- prod job PIDs, only on GPUs 0-3 UUIDs
1518126, 1393 MiB, GPU-e4fe48bc-0c29-f21e-6523-759f964bf823   <- ONLY our PID, ONLY on GPU 6
```

Peak for our PID was **4775 MiB at 41% GPU util** on GPU 6 — real on-GPU compute. Across all
in-band samples our PID touched **exactly one** GPU UUID (GPU 6); it never appeared on any
other GPU.

#### Multi-GPU (deferred)

Not run this wave — the box's other free GPUs were reserved for other agents and GPUs 0–3
were a production job. A 2- or 8-GPU pass on H100 should be **trivial** given the single-GPU
...
**WORKS on H100, single GPU, with no env-var workaround.** The only deviations from the
