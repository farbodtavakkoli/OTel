...
def parse_args():
    ...
    parser.add_argument('--test_data_path', default = 'data/classification_data.json',
                        help='Test data (JSON list) with prompt fields')
    parser.add_argument('--model_name', default = '9f0122ddc2e55a14f1ae195c871c736b0673d98d2f0cfcf77b85a5863b98b052',
                        help='Model id (job hash) of your fine-tuned model; to use the base model '
                             'instead, remove model_name from the llm.generate call')
    parser.add_argument('--max_tokens', default = 500,
                        help='Total input plus output tokens allowed, e.g. 4096 deployed context = '
                             '3000 input + 1096 max output')
    parser.add_argument('--batch_size', default = 2,
                        help='Number of sequences per batch (size it to fit max_tokens)')
    parser.add_argument('--inference_mode', default = 'language_model',
                        choices=['language_model', 'embedding'],
                        help='embedding models vLLM cannot serve can be embedded locally with '
                             'sentence-transformers from the base model instead')
    parser.add_argument('--model_type', default='rnj-1',
                        help='Chat-template family for special-token formatting (gemma3, qwen3, llama3, '
                             'rnj-1, olmo3, mistral, gpt-oss_reasoning, gpt_oss_it, lfm, phi4)')
